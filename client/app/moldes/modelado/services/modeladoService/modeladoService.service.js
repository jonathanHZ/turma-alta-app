...
angular.module('turmaAltaAppApp')
  .service('modeladoService', function () {
    ...
    this.calcular = function (largo, cantidadGajos, bainha) {
      var response = {
        distancia : [],
        ancho : [],
        largoacum : []
      ...
      //  }
       ...
       response.altoinf.value = Math.round(altoinf * largo / largoBase);
       response.anchoinf.value = Math.round((gajoMax / Math.PI) * largo / largoBase);
       response.boca.value = Math.round((circunferenciaBase[0] * largo / largoBase / Math.PI) * 10) / 10;
       response.gajomax.value = Math.round(((gajoMax * largo / largoBase / cantidadGajos)+(bainha*1)) * 10) / 10;
       response.superficie.value = Math.round(superficie * Math.pow(largo / largoBase, 2) * 10) / 10;
       response.volumen.value = Math.round(volumen * Math.pow(largo / largoBase, 3) * 10) / 10;
       ...
       for (var i = 0; i < distanciaBase.length; i++){
          ...
          response.distancia[i].value = Math.round(distanciaAux * 10) / 10;
          response.ancho[i].value = Math.round(anchoAux * 10) / 10;
          response.largoacum[i].value = Math.round(largoAcum * 10) / 10;
       }
